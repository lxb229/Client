...
import org.treediagram.nina.resource.other.Getter;
import org.treediagram.nina.resource.other.GetterBuilder;
import org.treediagram.nina.resource.other.IndexGetter;
import org.treediagram.nina.resource.other.InjectDefinition;
...
public class Storage<K, V> extends Observable implements ApplicationContextAware {
	...
	private void load() {
		...
		try {
			// 获取数据源
			String resourcePath = getLocation();
			//logger.error("正在加载:  " + resourcePath);
			Resource resource = applicationContext.getResource(resourcePath);
			input = resource.getInputStream();
			// 获取存储空间
			Iterator<V> it = reader.read(input, getClz(), resourceDefinition.getStartMark());
			clear();
			while (it.hasNext()) {
				V obj = it.next();
				// 注入 Spring 容器的内容
