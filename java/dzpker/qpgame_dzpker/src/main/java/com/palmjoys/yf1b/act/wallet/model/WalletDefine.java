...
public interface WalletDefine {
	...
	//推送消息(钱包数据变化)
	int WALLET_COMMAND_WALLET_NOTIFY = WALLET_COMMAND_BASE_NOTIFY + 1;
	
	
	//玩家不存在
	@SocketCode("您的钻石不足")
	int WALLET_ERROR_DIAMOND = WALLET_ERROR_BASE - 1;
	@SocketCode("您的房卡不足")
	int WALLET_ERROR_ROOMCARD = WALLET_ERROR_BASE - 2;
	@SocketCode("不能赠送房卡给自已")
	int WALLET_ERROR_GIVEROOMCARD = WALLET_ERROR_BASE - 3;
	@SocketCode("接口参数错误")
	int WALLET_ERROR_CMDPARAM = WALLET_ERROR_BASE - 4;
