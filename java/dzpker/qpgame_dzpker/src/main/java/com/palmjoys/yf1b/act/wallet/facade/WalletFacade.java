...
public interface WalletFacade {
	...
	@NetworkApi(value = WalletDefine.WALLET_COMMAND_ROOMCARD_GIVE, 
			desc="赠送房卡")
	...
	@NetworkApi(value = WalletDefine.WALLET_COMMAND_ROOMCARD_RECORD, 
			desc="获取房卡赠送记录")
	Object wallet_roomcard_record(@InSession Long accountId);
	
	@NetworkApi(value = WalletDefine.WALLET_COMMAND_WALLET_NOTIFY, 
			desc="推送消息(钱包数据变化)")
	Object wallet_wallet_notify();
}
