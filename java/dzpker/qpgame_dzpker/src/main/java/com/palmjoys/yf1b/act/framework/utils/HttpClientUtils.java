...
public class HttpClientUtils {
     ...
     * @return 响应字符串
     */
	public static HttpUtilsResponse executeByPost_Pay(String url, String params) {
    	//得到HttpClient实例
        HttpClient httpclient = getHttpClient();
        //创建post请求
        HttpPost post = new HttpPost(url);
        ...
        //ResponseHandler<String> responseHandler = new BasicResponseHandler();
        //返回结果
        HttpUtilsResponse retResponse = new HttpUtilsResponse();
        ...
        try {
            ...
            //执行请求并获取结果
            //responseJson = httpclient.execute(post, responseHandler); 
            responseObj = httpclient.execute(post);
            String responseString = EntityUtils.toString(responseObj.getEntity());
            retResponse.setResponseString(responseString);
            retResponse.setStatusCode(responseObj.getStatusLine().getStatusCode());
        } catch (Exception e) {
        	//logger.error("executeByPost---请求失败--------"+e.getMessage());
        } finally {
        	 //当不再需要HttpClient实例时,关闭连接管理器以确保释放所有占用的系统资源
