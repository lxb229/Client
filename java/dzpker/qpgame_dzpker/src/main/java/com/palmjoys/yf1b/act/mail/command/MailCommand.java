...
public class MailCommand {
	 ...
	 * */
	@ConsoleCommand(name = "gm_mail_send_systemail", description = "GM发送邮件给玩家")
	public Object gm_mail_send_systemail(String title, String content, String []recvPlayers){
		if(title.isEmpty()){
		...
		for(String sAccountId : recvPlayers){
			...
			MailEntity mailEntity = mailManager.loadOrCreate(theAccountId);
			mailEntity.lock();
			try{
			...
			//检查推送红点消息
			hotPromptManager.updateHotPrompt(theAccountId, HotPromptManager.HOT_KEY_MAIL);
			hotPromptManager.hotPromptNotity(theAccountId);
		}
