...
import com.palmjoys.yf1b.act.account.model.PhoneVaildCodeAttrib;
...
public class AuthenticationManager {
	...
	public AuthenticationEntity findOf_phone(String phone){
		long accountId = 0;
		//先查全表
		String querySql = "SELECT A.accountId FROM AuthenticationEntity AS A WHERE A.phone='" + phone + "'";
		List<Object> retObjects = querier.listBySqlLimit(AuthenticationEntity.class, Object.class, querySql, 0, 1);
		...
		if(0 != accountId){
			return this.loadOrCreate(accountId);
		}
		//后查缓存
		List<AuthenticationEntity> retEntitys = null;
		retEntitys = authenticationCache.getFinder().find(AccountFilterManager.Instance().createFilter_AuthenticationFilter_Phone(phone));
	...
	public AuthenticationVo getAuthenticationData(long accountId){
		AuthenticationVo retVo = new AuthenticationVo();
		AuthenticationEntity entity = this.loadOrCreate(accountId);
		retVo.name = entity.getName();
		retVo.cardId = entity.getCardId();
		retVo.phone = this.phoneEncryption(entity.getPhone());
