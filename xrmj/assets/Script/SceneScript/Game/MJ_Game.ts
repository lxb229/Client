...
export default class MJ_Game extends cc.Component {
     ...
     */
    @property(cc.Label)
    lblRemainingCard: cc.Label = null;
    /**
     * 当前局数
     * @type {cc.Label}
     * @memberof MJ_Game
 ...
 */
    showTableState() {
        ...
        if (dd.gm_manager.mjGameData.tableBaseVo.gameState > MJ_GameState.STATE_TABLE_DINGQUE
            ...
            this.node_game_info.active = true;
            this.lblRemainingCard.string = dd.gm_manager.mjGameData.tableBaseVo.tableCardNum.toString();
            this.lblCurGame.string = dd.gm_manager.mjGameData.tableBaseVo.currGameNum.toString();
            this.lblCountGame.string = dd.gm_manager.mjGameData.tableBaseVo.maxGameNum.toString();
            pIndex = dd.gm_manager.getIndexBySeatId(dd.gm_manager.mjGameData.tableBaseVo.btIndex);
        } else {
            this.node_game_info.active = false;
        }
            ...
            if (seatInfo && seatInfo.accountId && seatInfo.accountId !== '' && seatInfo.accountId !== '0') {
                ...
                if (seatInfo.outUnUseCards) {
                    let cardConfig: CardConfig = {
                        row: 14,
                        cScale: 1,
                        minSpacing: cc.v2(0, -8),
                        rightSpacing: cc.v2(2, -8)
                    };
    ...
    showHuPai(index: number, seatInfo: SeatVo) {
        ...
        if (node_hu) {
            ...
            if (seatInfo.accountId === dd.ud_manager.mineData.accountId) {
                let wr = 12;
                let holdLayout = cc.find('In_MineNode/holdLayout', node_hu.parent);
                if (holdLayout) {
                    wr = holdLayout.getComponent(cc.Widget).right - 112;
                }
                node_hu.getComponent(cc.Widget).right = wr;
            }
            ...
            if (seatInfo.huPaiType !== 0 && seatInfo.huCards) {
                ...
                for (var i = 0; i < seatInfo.huCards.length; i++) {
                    dd.gm_manager.getGMTarget().showPlayOutCard(sId, seatInfo.huCards[i], node_hu, (huNode: cc.Node) => {
                        if (seatInfo.accountId === dd.ud_manager.mineData.accountId) { //如果是自己
                            huNode.scale = 1.8;
                        }
                    });
                }
                if (node_img_hu) {
                    ...
                    switch (sId) {
                        case 1:
                            node_img_hu.rotation = -90;
                            break;
                        case 2:
                            node_img_hu.rotation = 180;
                            break;
                        case 3:
                            node_img_hu.rotation = 90;
                            break;
                        default:
                            node_img_hu.rotation = 0;
                            break;
                    }
