...
export default class MJ_Tang extends cc.Component {
    ...
    initData(tangCfg: TangCfg, cb) {
        ...
        if (tangCfg.cardIds && tangCfg.cardIds.length > 0) {
            ...
            if (this.nodeLayout.childrenCount > 1) {
                let dx = (13 - tangCfg.cardIds.length) * this.nodeLayout.children[0].width / (tangCfg.cardIds.length - 1);
                if (dx < 6) {
                    dx = 6;
                } else if (dx > 100) {
                    dx = 100;
                } else { }
                this.nodeLayout.getComponent(cc.Layout).spacingX = dx;
            }
        }
    }
