...
export default class MJ_PlayerUI extends cc.Component {
    ...
    onLoad() {
        this.node.on(cc.Node.EventType.TOUCH_END, (event: cc.Event.EventTouch) => {
            ...
            dd.gm_manager.getGMTarget().showRoleInfo(this._seatInfo.accountId);
    ...
    async showInfo(seatInfo: SeatVo) {
        ...
        if (dd.gm_manager.mjGameData.tableBaseVo.btIndex === seatInfo.seatIndex) {
            this.lblName.string = dd.utils.getStringBySize(this._seatInfo.nick, 8);
            if (dd.gm_manager.mjGameData.tableBaseVo.gameState > MJ_GameState.STATE_TABLE_FAPAI) {
                this.light.active = true;
            } else {
                this.light.active = false;
            }
        } else {
            this.lblName.string = this._seatInfo.score.toString();
            this.light.active = false;
        }
        ...
        switch (dd.gm_manager.mjGameData.tableBaseVo.cfgId) {
            ...
            default:
                ...
                if (this._seatInfo.unSuit > 0 && dd.gm_manager.mjGameData.tableBaseVo.gameState > MJ_GameState.STATE_TABLE_DINGQUE) {
                    this.unSuit.node.active = true;
                    this.unSuit.spriteFrame = dd.gm_manager.getGMTarget().unSuit_list[this._seatInfo.unSuit - 1];
                } else {
                    this.unSuit.node.active = false;
                }
                break;
