    ...
    private doPush(msgId: Protocol, content: any): void {
        switch (msgId) {
            ...
            case Protocol.ACCOUNT_NOTIFY_WALLET://玩家房卡数据
                UDManager.getInstance().mineData.wallet = content as Wallet;
                break;
            ...
            case Protocol.CORPS_ADD_MEMBER_NOTIFY://推送消息(俱乐部成员添加通知)
                ...
                break;
            case Protocol.CORPS_KICK_KMEMBER_NOTIFY:// 推送消息(俱乐部成员移除通知)
            ...
            case Protocol.CORPS_DESTORY_NOTIFY:// 推送消息(俱乐部解散通知)
                ...
                break;
            case Protocol.CORPS_TABLE_CREATE_NOTIFY:// 推送消息(帮会房间创建)
                cc.systemEvent.emit('Club_Msg_Push', content);
                break;
            case Protocol.CORPS_TABLE_DATA_CHANAGE_NOTIFY:// 推送消息(帮会房间数据更改)
                cc.systemEvent.emit('Club_Table_Push', content);
                break;
            case Protocol.CORPS_ZHANRANG_CORPS_NOTIFY://推送消息(帮会转让通知)
                cc.systemEvent.emit('Club_Attorn_Push', content);
                break;
            case Protocol.CORPS_NOTICE_MODFIY_NOTIFY://推送消息(帮会微信公告修改通知)
                cc.systemEvent.emit('Club_ModifyWX_Push', content);
                break;
            case Protocol.CORPS_WALLET_MODFIY_NOTIFY://推送消息(帮会钱包变化通知)
                cc.systemEvent.emit('Club_Wallet_Push', content);
                break;
            default: break;
        }
    }
