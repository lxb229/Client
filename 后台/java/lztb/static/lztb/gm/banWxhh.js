			...
			methods: {
				...
				gridFristLoad: function () { //表格首次加载(fetch data of first page -> set data -> initialize pager)
					...
					this.fetchData(function (rs) {
						app.grid.data = rs.entry;
						app.grid.total = rs.count;
						var pageCount = Math.ceil(rs.count/10);
						//alert(Math.ceil(aaa));
						app.grid.pageCount = pageCount;	
						
						gLayPageIni('pager', app.grid.curr_page ,pageCount , function (newPage) {
				...
				fetchData: function (cb) {
					...
					$.post("/gm/forbid/wxhhGameWords", options, function (rs) {
						console.log(rs);
						cb(rs);
					}, "json");
				},
