			...
			methods: {
				...
				gridFristLoad: function () { //表格首次加载(fetch data of first page -> set data -> initialize pager)
					...
					this.fetchData(function (rs) {
						app.grid.dataWin = rs.data.list[0].win;
						app.grid.dataLose = rs.data.list[0].lose;
				...
				fetchData: function (cb) {
					...
					$.post("/backstage/game/queryRtBunkoPageList", options, function (rs) {
						console.log(rs);
						cb(rs);
					}, "json");
				}
