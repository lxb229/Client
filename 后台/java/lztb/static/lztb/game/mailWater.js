		...
		var app = new Vue({
			...
			methods: {
				...
				fetchData: function (cb) {
					...
					$.post("/backstage/game/queryMailWaterPageList", options, function (rs) {
						console.log(rs);
						cb(rs);
					}, "json");
				},
				exportExcel: function(){
					var vm = this;
                    var url = "/backstage/game/exportMailWater?startDate=" + vm.filter.startDate + "&endDate=" + vm.filter.endDate;
				...
				delData: function (gmdId) {
					...
					ConfirmBox(msg, function () {
						$.ajax({
							url: '/backstage/game/deleteMailWater',
							type: 'POST',
							data: { 'gmdId': gmdId },
							dataType: 'json',
							timeout: 5000,
			...
			mounted: function () {
				...
				//提交  
                form.on('submit(mail_form)', function (data) {
                    
                    var vm = this;
                    app.filter = data.field; 
                    app.gridFristLoad();
                    return false; //阻止原生表单跳转
                });
                form.render();
