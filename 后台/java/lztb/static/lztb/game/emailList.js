			...
			methods: {
				gridFristLoad: function () { //表格首次加载(fetch data of first page -> set data -> initialize pager)
					...
					this.fetchData(function (rs) {
						//alert(JSON.stringify(rs.data.list[0].rece));
						app.grid.dataRece = rs.data.list[0].rece;
						app.grid.dataSend = rs.data.list[0].send;
				...
				fetchData: function (cb) {
					...
					$.post("/backstage/game/queryEmailListPageList", options, function (rs) {
						console.log(rs);
						cb(rs);
					}, "json");
				}
