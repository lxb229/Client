...
public class PlayerController extends BaseController {
    ...
    public PageInfo dataGrid(Player player, Integer page, Integer rows, String sort,String order) {
        ...
        if (StringUtils.isNotBlank(player.getPlayId())) {
            condition.put("playId", player.getPlayId());
        }
    ...
    @PostMapping("/add")
    @ResponseBody
    public Object add(@Valid Player player) {
        boolean b = playerService.insert(player);
        if (b) {
    ...
    @PostMapping("/delete")
    @ResponseBody
    public Object delete(int id, int status) {
        ...
        Result result = playerService.deletePlayer(player);
        if (result.isSuccess()) {
            return renderSuccess(result.getMsg());
        } else {
