...
public interface BaseMapper<T, PK extends java.io.Serializable> {
	 ...
	 * @return List<T>
	 */
	List<T> selectPageByParam(@Param("bParam") T bParam, @Param("pParam") PageResult<T> pParam, @Param("user") Users user);
	/**
	 * 通用分页总数查询
	 * @Title: countByParam 
	 * @param @param bParam
	 ...
	 * @return Integer
	 */
	Long countByParam(@Param("bParam") T bParam, @Param("user") Users user);
	/**
	 * 通用查询
	 * @Title: select 
	 * @param @param bParam
