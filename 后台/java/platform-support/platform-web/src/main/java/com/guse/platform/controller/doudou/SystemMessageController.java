...
public class SystemMessageController extends BaseController {
    ...
    public @ResponseBody Object queryPagelist(SystemMessage message, PageBean pageBean) throws Exception {
    	...
    	Result<PageResult<SystemMessage>> result = null;
		try {
			result = service.queryPageList(pageBean, message, null, user);
		} catch (Exception e) {
    	...
    	for (int i = 0; i < result.getData().getList().size(); i++) {
    		SystemMessage systemMessage = result.getData().getList().get(i);
    		Users createUsers = usersMapper.selectByPrimaryKey(systemMessage.getCreateId());
    		systemMessage.setCreateUser(createUsers);
    		Users smUser = usersMapper.selectByPrimaryKey(systemMessage.getSmUserId());
    		systemMessage.setSmUser(smUser);
    		result.getData().getList().set(i, systemMessage);
    	}
