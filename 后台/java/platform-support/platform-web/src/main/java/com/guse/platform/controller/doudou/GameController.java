...
public class GameController extends BaseController {
    ...
    public @ResponseBody Object queryLuckyWheelPageList(PageBean pageBean) throws Exception{
    	...
    	Result<PageResult<GameActivityRotorStatistics>> result=gameActivityRotorStatisticsService.queryLuckyWheelPageList(pageBean,user);
    	if (!result.isOk()) {
    		return new AjaxResponse(Constant.CODE_ERROR, result.getErrorMsg());
    	}
    	return new AjaxResponse(result.getData());
    ...
    public @ResponseBody Object queryLuckyWheelDetailPageList(GameActivityRotorDetail gameActivityRotorDetail,PageBean pageBean) throws Exception{
    	Users user   = (Users) getSession().getAttribute(Constant.SESSION_LOGIN_USER);
    	Result<PageResult<GameActivityRotorDetail>> result=gameActivityRotorDetailService.queryLuckyWheelDetailPageList(gameActivityRotorDetail,pageBean,user);
    	if (!result.isOk()) {
    		return new AjaxResponse(Constant.CODE_ERROR, result.getErrorMsg());
    	}
    	return new AjaxResponse(result.getData());
