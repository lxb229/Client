...
public class LoginController extends BaseController {
     ...
     * @return
     */
    @RequestMapping(value = "/system/user/sendCode", method = RequestMethod.POST)
    public @ResponseBody Object sendCode(String phone) {
    	Random random=new Random();
		String smsCode = Integer.toString(random.nextInt(900000)+100000);
		boolean success = gameGMUtil.sendSMS(phone, smsCode);
    	if (!success) {
    		return new AjaxResponse(Constant.CODE_ERROR, "验证码发送不成功");
    	}
    	Result<Integer> result = userService.sendVerCode(phone, smsCode);
    	if(!result.isOk()) {
    		return new AjaxResponse(Constant.CODE_ERROR, "验证码写入出错");
    	} else {
    		return new AjaxResponse(Constant.CODE_SUCCESS, "验证码写入出错");
    	}
    }
