...
public abstract  class BaseServiceImpl<T,PK extends java.io.Serializable> implements BaseService<T,PK> {
	...
	@Override
	public Result<PageResult<T>> queryPageList(PageBean pageBean,T t,String orderBy,Users user) {
		Long count = baseMapper.countByParam(t, user);
		if (count <= 0) {
             return new Result<PageResult<T>>(new PageResult<T>().initNullPage());
        }
		List<T> list = baseMapper.selectPageByParam(t, new PageResult<T>(pageBean.getPageNo(), pageBean.getPageSize(), count, orderBy), user);
