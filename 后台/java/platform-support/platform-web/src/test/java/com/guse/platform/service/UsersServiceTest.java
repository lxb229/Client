...
public class UsersServiceTest extends TestBase {
	...
	public void queryPageList(){
		Users roleUser   = userService.getUserByAccountId(0L);
		Users user = new Users();
		user.setLoginName("admin");
		try {
			
			Result<PageResult<Users>> result = usersTestService.queryPageList(new PageBean(), user, "",roleUser);
//			Result<PageResult<Users>> result = userService.queryPageListByUsers(null, new PageBean());
	...
	public void queryPageListUser(){
		...
		user.setLoginName("admin");
		try {
			Result<PageResult<Users>> result = userService.queryPageListByUsers(null, new PageBean(),roleUser);
			System.out.println(JSON.toJSONString(result));
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	public void queryPageListUserCondition(){
		...
		user.setLoginName("admin");
		try {
			Result<PageResult<Users>> result = userService.queryPageListByUsers(user, new PageBean(),roleUser);
			System.out.println(JSON.toJSONString(result));
		} catch (Exception e) {
			e.printStackTrace();
		}
