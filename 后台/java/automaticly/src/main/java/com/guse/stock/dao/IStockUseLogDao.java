...
public interface IStockUseLogDao {
	 ...
	 */
	@Insert("insert into pl_stock_uselog(number, stock_id, user_id, status, poundage, mark, create_time) "
			+ "values(#{number}, #{stock_id}, #{user_id}, #{status}, #{poundage}, #{mark}, #{create_time})")
