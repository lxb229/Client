...
import { mp_manager } from './ModuleManager';
/**
 * 管理公共UI的类
 * 
 * @export
...
export default class UIManager {
    ...
    private cb_before_scene_loading: (event: cc.Event.EventCustom) => void = (event: cc.Event.EventCustom) => {
        ...
        let seneceName = cc.director.getScene().name;
        if (seneceName === 'ZJHScene' || seneceName === 'NNScene') {
            mp_manager.playBackGround();
        }
    }
    /**
     * 场景切换后的回调
    ...
    setLoading(msg: string) {
        if (this.isShowLoading) {
            let loading: Loading = this.getCanvasNode().getChildByName('Loading').getComponent('Loading') as Loading;
            loading.setMsg(msg);
        }
    }
     ...
     * @memberof UIManager
     */
    showAlert(msg: string, title: string, obj_yes?: btn_obj, obj_no?: btn_obj, ha: number = 0) {
        this.hideLoading();
