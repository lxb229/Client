...
export default class WSManager {
    ...
    */
        ...
        return new Promise<void>((resolve, reject) => {
            ...
            this.ws.onmessage = (ev: MessageEvent) => {
                ...
                if (msgId > 10000) {
                    ...
                    if (sendData) {
                        let oldTime = this.delayTime;
                        this.delayTime = Date.now() - sendData.sendTime;
                        if (oldTime !== this.delayTime) {
                            cc.systemEvent.emit('msDelay', this.delayTime);
                        }
     ...
     * 心跳检测
     * 
     * @param {number} dateTime 当前时间（毫秒）
     * @memberof WSManager
     */
    private checkHeartbeat(dateTime: number) {
    ...
    private doPush(msgId: Protocol, content: any): void {
        switch (msgId) {
            ...
            case Protocol.ACCOUNT_NOTIFY_WALLET://玩家房卡数据
                ...
                cc.systemEvent.emit('WalletPush', content);
                break;
            case Protocol.ORDER_RMB2GOLDMONEY_NOTIFY://玩家充值房卡数据推送
                ui_manager.showAlert('您充值的' + content + '金币已到账！', '充值通知', null, null, 1);
                break;
            case Protocol.ZJH_GAMESTATE_CHANAGE_NOTIFY://推送消息(游戏状态变化)
                cc.systemEvent.emit('GamePush', content);
                break;
            case Protocol.ZJH_SEAT_CHANAGE_NOTIFY://推送消息(座位数据变化)
                cc.systemEvent.emit('SeatPush', content);
                break;
            case Protocol.ZJH_TABLE_BET_NOTIFY://推送消息(下注数据变化)
                cc.systemEvent.emit('BetPush', content);
                break;
            case Protocol.ZJH_TABLE_KICK_PLAYER_NOTIFY://推送消息(玩家被踢出座位)
                cc.systemEvent.emit('KickPush', content);
                break;
            case Protocol.ZJH_TABLE_NN_CALLBANKER_NOTIFY://牛牛叫庄
                cc.systemEvent.emit('CallBanker', content);
                break;
            case Protocol.ZJH_TABLE_LOOKCARD_NOTIFY://推送消息(玩家看牌数据)
                cc.systemEvent.emit('LookPush', content);
                break;
            case Protocol.ZJH_TABLE_NN_GAMEREADY_NOTIFY://牛牛准备
                cc.systemEvent.emit('Ready', content);
                break;
            default: break;
        }
    }
